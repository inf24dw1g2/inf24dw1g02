...
async function initDB() {
  try {
    ...
    await connection.query('DROP TABLE IF EXISTS meal_plans');
    ...
    if (userRows[0].count === 0) {
      ...
      `);
      console.log("✅ Inseridos 30 usuários");
    }

    // Inserting 30 workouts
    await connection.query(`
    ...
    `);
    console.log("✅ Inseridos 30 treinos");

    // Inserting 30 progress records
    await connection.query(`
      INSERT INTO progress (user_id, date, weight, chest, arms, legs)
      SELECT id, CURDATE(), weight + FLOOR(RAND()*5), 90 + FLOOR(RAND()*6), 35 + FLOOR(RAND()*3), 60 + FLOOR(RAND()*5) FROM users ORDER BY RAND() LIMIT 30
    `);
    console.log("✅ Inseridos 30 registos de progresso");

    // Inserting 30 meal plans
    await connection.query(`
      ...
      FROM users ORDER BY RAND() LIMIT 30
    `);
    console.log("✅ Inseridos 30 planos alimentares");
